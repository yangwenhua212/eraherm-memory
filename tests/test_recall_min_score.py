...
def test_recall_min_score_filters_weak_hits(tmp_path: Path) -> None:
    ...
    related = svc.recall(user_id="u_ms", query="螺蛳粉 炸蛋 喜好", top_k=5)
...
def test_recall_api_min_score_empty(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with TestClient(app) as client:
        client.post(
            "/v1/memories",
            json={"user_id": "u_api", "content": "老大喜欢吃螺蛳粉", "importance": 1.0},
        )
        res = client.post(
            "/v1/recall",
        ...
        res2 = client.post(
            "/v1/recall",
            json={"user_id": "u_api", "query": "螺蛳粉", "top_k": 5, "min_score": 0.0},
        )
        assert res2.status_code == 200
        assert any("螺蛳粉" in i["content"] for i in res2.json()["items"])
    get_settings.cache_clear()
