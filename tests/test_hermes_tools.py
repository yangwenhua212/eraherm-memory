...
def test_dispatch_remember_recall_correct(tools: HermesMemoryTools) -> None:
    remembered = json.loads(
        tools.dispatch(
            "memory_remember",
            {"content": "老大喜欢吃螺蛳粉加炸蛋", "pinned": True, "importance": 0.95},
        )
    )
    assert remembered["ok"] is True
    assert remembered["id"]

    recalled = json.loads(tools.dispatch("memory_recall", {"query": "螺蛳粉"}))
    assert recalled["ok"] is True
    assert recalled["count"] >= 1
    assert any("螺蛳粉" in i["content"] for i in recalled["items"])

    corrected = json.loads(
        tools.dispatch(
            "memory_correct",
            {"correction": "老大喜欢螺蛳粉加炸蛋不是卤蛋", "wrong_answer": "卤蛋"},
        )
    )
