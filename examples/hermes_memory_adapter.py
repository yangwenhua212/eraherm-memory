...
"""Hermes ↔ EraHerm-Memory Host adapter (copy/adapt into Hermes).

This is NOT the memory kernel. It encodes *when* Hermes should call the kernel
and how to inject recall into the prompt.

Usage (needs running API):
  uvicorn app.main:app --port 8000
  python examples/hermes_memory_adapter.py

Or import HermesMemoryBridge in your Hermes repo and wire it into the turn loop.
"""
...
# Heuristics: what is worth writing to long-term memory from a user utterance.
...
class HermesMemoryBridge:
    ...
    def after_turn(
        ...
        *,
        ...
        # Explicit correction from Hermes UI / user
        ...
        # Soft detect correction in the *next* user message — caller may pass it as user_text
    ...
    def build_system_suffix(self, ctx: TurnContext) -> str:
        """Hermes can append this to the system prompt / tool preamble."""
...
def format_recall(recall: dict[str, Any]) -> str:
    ...
    for it in items:
        ...
        lines.append(f"- ({it.get('layer')}, score={it.get('score', 0):.2f}){pin} {it.get('content')}")
    return "\n".join(lines)
...
def format_recommendations(recall: dict[str, Any]) -> str:
    ...
    for r in recs:
        lines.append(f"- ({r.get('reason')}, score={r.get('score', 0):.2f}) {r.get('content')}")
    return "\n".join(lines)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Hermes memory adapter demo")
